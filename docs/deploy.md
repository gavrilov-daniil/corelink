...
2. При необходимости включить вход по Telegram (экран «Доступ» → «Вход по Telegram»). В BotFather: Bot Settings → Web Login, добавить в **Allowed URLs** адрес `https://$ADMIN_PUBLIC_HOST/api/admin/auth/telegram/callback` и взять оттуда же `Client ID` и `Client secret` — их и вписать в админку вместе с тем же redirect URI. Telegram сверяет адрес посимвольно: расхождение в схеме, порту или пути ломает вход. Незнакомый Telegram-аккаунт после входа попадает в заявки и доступа не получает, пока его не подтвердят с выбором роли.
